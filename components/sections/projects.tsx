...
export function ProjectsSection() {
  ...
  return (
    <section 
      id="projects" 
      ref={targetRef} 
      className="relative h-[300vh] bg-card/5 border-t border-border/30"
    >
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div className="container-tight mb-8">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-section/20 bg-section-muted text-section text-xs font-mono uppercase tracking-wider mb-4">
        ...
        >
          {PROJECTS.map((project) => (
            <TiltCard key={project.id} className="w-[85vw] max-w-[800px] shrink-0 h-full">
              <div 
                className="w-full h-full rounded-3xl border border-border/50 bg-background/80 backdrop-blur-md p-6 sm:p-10 shadow-2xl flex flex-col"
              >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 pb-5">
                ...
                <a
                  ...
                  className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border/50 bg-card/40 hover:bg-card px-4 text-xs font-semibold text-foreground transition-all"
                >
                  <Code className="h-3.5 w-3.5" />
                  GitHub Repo
              ...
              <div className="mt-6 flex-1 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="rounded-xl border border-brand-muted bg-section-muted/50 p-5 relative overflow-hidden flex flex-col justify-center">
                    ...
                    <p className="text-xs sm:text-sm text-section/90 leading-relaxed">
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-border/40 pt-6 mt-6">
                {project.metrics.map((m) => (
                  <div key={m.label} className="text-left border-r border-border/10 last:border-0 pr-4">
                    <span className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider block mb-1">
                      {m.label}
                    </span>
                    <span className="text-lg sm:text-2xl font-bold text-section block">
                      {m.value}
                    </span>
                  </div>
                ))}
